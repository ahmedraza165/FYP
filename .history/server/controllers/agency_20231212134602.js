...
const paidEnrollment = async (req, res) => {
  try {
    ...
    const deadlineDate = new Date(package.deadline);

    if (currentDate > deadlineDate) {
      // Display toast.error if the deadline has passed
      return res.status(400).send("Enrollment failed. Deadline has passed.");
    }
...
//  {Get}
const getChatbewtweenUserAndAgency = async (req, res) => {
  try {
    const { data } = req.params;
    const { packageId } = req.query;

    console.log('UserId', data, 'PackageId', packageId);

    // Use async/await to handle the asynchronous operation of querying the database
    const chatData = await Chat.findOne({ user: data, package: packageId }).select('message');

    console.log(chatData);

    if (chatData) {
      res.status(200).json(chatData.message);
    } else {
